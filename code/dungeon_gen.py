...
class DungeonGenerator:
	...
	def generate(self):
		...
		self._split_area(0, 0, self.width, self.height, 0)
	...
	def _split_area(self, x, y, w, h, depth):
		# Stop splitting if max depth reached or area too small for a room plus padding
		if depth >= self.max_depth or w < 2 * self.min_room_size - 1 or h < 2 * self.min_room_size - 1:
		...
		if split_horizontally:
			...
			self._split_area(x, y, w, split_point, depth + 1)
			self._split_area(x, y + split_point, w, h - split_point, depth + 1)
		else:
			# Vertical split
			...
			self._split_area(x, y, split_point, h, depth + 1)
			self._split_area(x + split_point, y, w - split_point, h, depth + 1)
	...
	def _create_room(self, x, y, w, h):
		...
		self.rooms.append({'x': room_x, 'y': room_y, 'w': room_w, 'h': room_h,
	...
	def _create_corridors(self):
		...
		for i in range(len(self.rooms) - 1):
			...
			if random.choice([True, False]):
				...
				self._h_line(start_x, end_x, start_y)
				self._v_line(start_y, end_y, end_x)
			else:
				# Move vertically then horizontally
				self._v_line(start_y, end_y, start_x)
				self._h_line(start_x, end_x, end_y)
