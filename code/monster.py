...
class VisionCone:
    ...
    def draw(self, surface, offset):
        ...
        points = []
        for angle in angles:
            points.append(self._cast_ray(angle, edges))
    ...
    def ray_cast(self, origin, target_pos, edges=None):
        if edges is None: edges = self._get_edges()
        ox, oy = origin
        tx, ty = target_pos
        dx, dy = tx - ox, ty - oy
    ...
    def _cast_ray(self, angle, edges):
...
class Monster(Entity):
    ...
    def __init__(self, pos, group, grid, monster_type, static_edges, player, hideable_sprites):
        ...
        # timers
        self.timers = {'action': Timer(random.randint(1000, 3000), self._change_action), 'hear_cooldown': Timer(1000)}
    ...
    def update(self, dt):
        ...
        self._check_hearing(self.player)
        self._check_player_caught(self.player)
    ...
    def _check_hearing(self, player):
        if self.state == 'CHASE': return
        if player.sound_radius > 0:
            dist = (self.pos - player.pos).magnitude()
            if dist <= player.sound_radius: self._hear_sound(player.pos)

    def _check_player_caught(self, player):
    ...
    def _hear_sound(self, sound_pos):
